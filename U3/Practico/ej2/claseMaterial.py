class mate_refrec:
  __idd: int
  __material: str
  __cant_utilizada: int
  __costo_adicional:int
  __caracteristicas: str
  
  def __init__(self,idd:int, material:str,cant_utilizada:int,costo_adicional:int,caracteristicas:str):
    self.__idd = idd
    self.__material = material
    self.__caracteristicas = caracteristicas
    self.__cant_utilizada = cant_utilizada
    self.__costo_adicional = int(costo_adicional)
    
  @property
  def material(self):
    return self.__material

  @property
  def caracteristicas(self):
    return self.__caracteristicas

  @property
  def cant_utilizada(self):
    return self.__cant_utilizada

  @property
  def costo_adicional(self):
    return int(self.__costo_adicional)

  @property
  def idd(self):
    return int(self.__idd)
  
  def __str__(self):
    return f"{self.__material}  "
